...
execute store result score #ench_lvl_update ng run data get storage ng:tmp mainhand.tag.StoredEnchantments[0].lvl

execute if score #ench_lvl_origin ng > #ench_lvl_update ng run return 1
$execute if score #ench_lvl_origin ng < #ench_lvl_update ng run data modify storage ng:tmp offhand.tag.Enchantments[$(index)].lvl set from storage ng:tmp mainhand.tag.StoredEnchantments[0].lvl
